...
sap.ui.define([
...
], function (Controller, xlsx, Filter, FilterOperator,JSONModel) {
	"use strict";
	...
	return Controller.extend("com.org.excelupload.uploadXl.controller.View1", {
		...
		_downloadData: function () {
			...
			var sToDate = this.getView().getModel("excelData").getProperty("/toDate");
			sToDate = sToDate.split("-").join("");
			var aFilters = [new Filter('fromDate', FilterOperator.EQ, sFromDate), new Filter('toDate', FilterOperator.EQ, sToDate)];
			// this.getView().byId("resultTable").getBinding("items").filter(new Filter({
			// 	filters: aFilters,
			// 	and: true
			// }));
			this.getOwnerComponent().getModel().read("/FUEL_SURCHRG_HEADERSet",{
				expand:"NAV_HEADER_TO_PRINT",
				filters:aFilters,
				success:function(oData,oResponse){
					var oOutPut = oData.results[0].NAV_HEADER_TO_PRINT;
